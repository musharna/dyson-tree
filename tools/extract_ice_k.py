...
#
# `--table` additionally writes sim/spectral_table.csv (the full 280-4000 nm basis Q4
# consumes) by calling tools/make_spectral_table.py, which needs ASTMG173.csv too.
# This file stays the PAR-only reading it has always been; the table has its own
# producer so the formatter and the provenance header live in one place.
...
def r_max(sig, p, k, tau):
    return 2 * sig * abs(math.log(tau)) / (k * p)
...
if "--table" in sys.argv:
    print("\n=== writing sim/spectral_table.csv (tools/make_spectral_table.py) ===")
    import importlib.util
    from pathlib import Path

    _p = Path(__file__).resolve().parent / "make_spectral_table.py"
    _s = importlib.util.spec_from_file_location("make_spectral_table", _p)
