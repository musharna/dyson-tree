...
for f in index.html spectral_table.js model.js map_data.js mapview.js app.js deck.js picture.js verdict.js; do
  test -f "web/$f" || { echo "missing web/$f" >&2; exit 1; }
  cp "web/$f" "site/$f"
done

# The deck table (web/deck.json, mirrored by sim/deck.py) as a classic script, so the page
# works from file:// where fetch() of a JSON file is refused. Written here, never by hand.
test -f web/deck.json || { echo "missing web/deck.json" >&2; exit 1; }
python3 -c 'import json,sys; json.load(open("web/deck.json"))' || { echo "web/deck.json is not JSON" >&2; exit 1; }
{ printf '// GENERATED by tools/build_site.sh from web/deck.json; do not edit.\nwindow.DysonDeckRows = '; cat web/deck.json; printf ';\n'; } > site/deck_rows.js
