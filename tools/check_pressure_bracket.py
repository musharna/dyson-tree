...
#
# !! VERDICT SUPERSEDED 2026-09-04 by section 13 S7 -- read that before citing this output. !!
# Every assertion below still holds and this script still passes: it correctly computes p_sat
# at T_interior = (N+1)^0.25 * T_eq. What is wrong is that PREMISE, which assumes an
# SW-TRANSPARENT wall. A pressure-bearing ice wall transmits only 16-23% of solar energy, so
# T_interior = (1+tau)^0.25 * T_eq and the self-consistent pressure is 2.6-3.5 kPa -- INSIDE
# the registered bracket, not 1.8x above it. The "framing A BINDS" line printed below is
# withdrawn; framing A is split by ice strength. See tools/check_greenhouse_transparency.py,
# which solves the fixed point this script's conclusion assumed away.
import signal
import sys
import math
...
t_un = (
    equilibrium_temperature(1.0, area_ratio=4.0) - 273.15
)  # algal sphere, UNCONTAINED
assert abs(t_un - 5.16) < 0.02, (
    f"uncontained T control failed: {t_un:.3f} vs published 5.16 C"
)
p_un = buck_pa(t_un)
assert abs(p_un - 882) < 3, f"882 Pa control failed: {p_un:.1f}"
t_c1 = (
    equilibrium_temperature(1.0, area_ratio=4.0) * (1 + 1) ** 0.25
) - 273.15  # + one IR-opaque shell
assert abs(t_c1 - 57.82) < 0.05, (
    f"contained T control failed: {t_c1:.3f} vs published 57.82 C"
)
p_c1 = buck_pa(t_c1)
...
print(
    f"             contained N=1 {t_c1:.2f} C -> {p_c1 / 1000:.1f} kPa (published 18)\n"
)
...
for N in range(0, 4):
    ...
    p = buck_pa(t)
    print(
        f"{N:8d} {t:10.2f} {p / 1000:16.3f} {('YES' if 882 <= p <= 10e3 else 'NO -- ABOVE TOP'):>13}"
    )
...
def r_max_km(s, p, k):
    return 2.0 * s * abs(math.log(0.01)) / (k * p) / 1000.0


for lab, p in [
    ("882 Pa (bracket floor, UNCONTAINED T)", p_un),
    ("10 kPa (bracket top)", 10e3),
    ("18.0 kPa (SELF-CONSISTENT, N=1)", p_c1),
]:
    lo, hi = r_max_km(0.7e6, p, 0.38623), r_max_km(3.1e6, p, 0.38623)
    print(
        f"  {lab:40s} {lo:8.2f} - {hi:7.2f} km   {'BINDS at 10 km' if hi < 10 else 'does not bind'}"
    )
