...
> was run, what it printed, and what was decided during the 1.0 release — as of
> that release. It is NOT the authority on any result: that is
> [`docs/FINDINGS.md`](FINDINGS.md), and where the two disagree FINDINGS is
> current and this file is history. Defects listed here as open or surviving may
> since have been fixed; the ones closed in 1.0.1 are annotated inline, and
> [`docs/RELEASE-1.0.1.md`](RELEASE-1.0.1.md) is that release's own log.
...
> ⚠️ That plan file is **not in the repository**: it was an internal executor
> brief, removed before release in `2ac9bb4` ("docs: remove the internal executor
> brief"). The citation is kept because it records what this log was written
> against, but a reader cannot open it. Everything it required is reproduced here
> as output. (Noted 1.0.1 — same defect class as the prereg citation fixed as
> item 9 of that release.)
...
> ✅ **Repaired in 1.0.1** — see *For the coordinator* item 3 below. The re-run was
> authorised and performed; the data came back byte-identical as predicted, and
> FINDINGS' *A note on the CSV provenance headers* now records a closed defect
...
3. `README.md` references `_pm/` and three private Claude skills — repo-external
   ...
   > ✅ **Fixed by the coordinator before the flip**, after this log was written.
   > The shipped `README.md` contains no `_pm/` reference and no private tool
   > name; `grep -n "_pm/" README.md` returns nothing. This entry describes the
   > state at the time of the critic pass, not the released tree.
...
3. `README.md` references `_pm/` and three private tool names — repo-external
   ...
   > ✅ **Fixed by the coordinator before the flip**, after this log was written.
   > The shipped `README.md` contains neither; this entry describes the state at
   > the time of the critic pass, not the released tree.
